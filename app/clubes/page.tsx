...
export default function ClubsDirectoryPage() {
  ...
  const filteredClubs = useMemo(() => {
    return CLUBS_DATA.filter((club: Club) => {
      ...
      const matchesSearch =
        ...
        (club.meetingPlace?.toLowerCase().includes(term) ?? false);
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-3 max-w-2xl">
              ...
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
                Conoce nuestras <strong className="text-[#00246C]">{CLUBS_DATA.length} sedes rotarias</strong> ubicadas desde Arica hasta Valparaíso y Rapa Nui. Encuentra tu club más cercano y súmate al servicio comunitario.
              </p>
            </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-8 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            <div className="md:col-span-7 relative">
              ...
              <input
                type="text"
                placeholder="Buscar por club, ciudad o presidente..."
                value={searchTerm}
                  ...
                  <div className="space-y-2 text-xs text-slate-600">
                    ...
                    {club.meetingDay && (
                      <p className="flex items-center gap-2">
                        ...
                        <span>Reuniones: <strong>{club.meetingDay}</strong> {club.meetingTime ? `(${club.meetingTime})` : ''}</span>
                      </p>
                    )}

                    {club.meetingPlace && (
                      <p className="flex items-center gap-2 text-slate-500 text-[11px] truncate">
                        <Clock className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                        <span className="truncate">{club.meetingPlace}</span>
                      </p>
                    )}
                  </div>
                </div>
