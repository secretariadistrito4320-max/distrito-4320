...
import { getLettersByGovernor } from '@/lib/getLetters';
...
export default async function GovernorDetailPage({ params }: GovernorPageProps) {
  ...
  // Obtener cartas dinámicas desde Google Sheets / AppSheet
  const letters = await getLettersByGovernor(gov.slug);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-200">
            ...
            <span className="text-xs font-bold text-[#00246C] bg-blue-50 px-3 py-1 rounded-lg border border-blue-200">
              {letters.length} Cartas Emitidas
            </span>
          </div>
          ...
          {letters.length === 0 ? (
            <div className="bg-white rounded-3xl p-10 text-center border border-slate-200 space-y-2">
              ...
              <h3 className="font-bold text-slate-700 text-sm">Sin cartas cargadas aún</h3>
              <p className="text-xs text-slate-500">
                Las cartas mensuales de este periodo se irán publicando paulatinamente.
              </p>
            </div>
          ) : (
            letters.map((letter) => (
